...
/* 3.
	...
	* 100 kg = 220.4622622 pounds (arredondando, os itens de 220 também serão inclusos)
*/
SELECT
...
SELECT * FROM DimStore WHERE CloseDate IS NULL
...
*/
SELECT * FROM DimStore WHERE EmployeeCount BETWEEN 1 AND 20 AND CloseDate IS NULL

SELECT * FROM DimStore WHERE EmployeeCount BETWEEN 21 AND 50 AND CloseDate IS NULL

SELECT * FROM DimStore WHERE EmployeeCount > 50 AND CloseDate IS NULL
...
FROM
	DimProduct
WHERE
	ProductName LIKE '%LCD%' AND ProductName LIKE '%TV%'
...
FROM
	DimProduct
WHERE
	BrandName = 'Contoso' AND ColorName = 'Silver' AND UnitPrice >= 10 AND UnitPrice <= 30
ORDER BY
	UnitPrice DESC
